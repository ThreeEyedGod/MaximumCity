...
_getTownNameWeatherFromTown town = (PirateWeatherAPI.getWeatherForTown $ Data.ByteString.Char8.unpack town) >>= (\w -> (getWaterLakeLevelForPlace_LiveToday_wrtStorage town >>=  _helperLivePercent) >>= (\wl w -> _handlePirateResponse w town $ )
