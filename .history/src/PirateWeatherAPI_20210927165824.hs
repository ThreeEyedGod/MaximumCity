...
_getTownNameWeatherFromTown town =  (getWaterLakeLevelForPlace_LiveToday_wrtStorage town 
>>=  _helperLivePercent) >>=
  (\wl -> ((PirateWeatherAPI._getWeatherForTown $ Data.ByteString.Char8.unpack town) >>= 
    (\w -> _handlePirateResponse w town wl)))

_preProcess:: String -> IO (LatLong , Either String Key) 
_preProcess town = getLatLongforThis town :: LatLong >>= (\a ->  getPirateWeatherSettings) -> (\z -> pure $ z ))
