  ...
  do
    case eitherDecode (LB.fromStrict (T.encodeUtf8 (fromMaybe "" body))) of
      ...
      Right update -> do
        ...
        pure $ Right $ H.Response 200 responseHeaders responseBody False
  where
    responseHeaders = [("Access-Control-Allow-Headers","*"), ("Content-Type","application/json"), ("Access-Control-Allow-Origin","*") :: String), "Access-Control-Allow-Methods" .= ("POST,GET,OPTIONS" :: String)]
