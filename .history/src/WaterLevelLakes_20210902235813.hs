...
getWaterLakeLevelForPlace_LiveToday_wrtStorage pl = do
            nrstRegion <- BSU.fromString $ findNearestRegionToPlace (T.unpack pl)
            x <- extractDivisionData nrstRegion $ getAllDivData "Major"
            case x of 
                Just y -> return $ (nrstRegion , getPercentLiveToday y
                Nothing -> error "Ouch "
