...
extLatLon str =  (x , y) where
    -- let x :: Angle  =  degree 19
    -- let y :: Angle  = degree 20
...
regionLatLong = [
("Aurangabad", mkGeo (degree 19.88461, degree 75.33816)), 
("Amravati", mkGeo (degree 20.95085, degree 77.75075)),
("Kokan", mkGeo (degree 17.22625, degree 75.33816)), 
("Nagpur", mkGeo (degree 21.15655, degree 79.08074)),
("Nashik", mkGeo (degree 19.99482, degree 73.79970)), 
("Pune", mkGeo (degree 18.51610, degree 73.86295))
]

findNearestRegionToPlace :: String -> IO String
findNearestRegionToPlace pl = do
    c <- getLatLongforThis pl 
    let (plLat, plLon) = extLatLon c
    let g = mkGeo (plLat, plLon)
    let di = map (\(str, to) -> (str , distance g to)) regionLatLong
    let onlydi = map (\x -> snd x) di
    let mindi = minimum onlydi
