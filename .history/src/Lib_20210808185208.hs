...
mkRespBodygetUpdate :: Event -> IO (T.Text, Maybe Update)
mkRespBodygetUpdate Event {path, headers, body} = do
    (h, b) <- getHdrBdyMaybe Event {path, headers, body} 
    responseBody <- getWeather (Just h, gettheTelegramMaybe b)
    pure (responseBody, b)
 
prepHdrBdy :: Event -> (LB.ByteString, Maybe T.Text) 
prepHdrBdy Event {path, headers, body} = (preProcessHeaders headers, 
          gettheTelegramMaybe $ 
          TLO.decode $ (LB.fromStrict (T.encodeUtf8 (fromMaybe "" body))))

getHdrBdyMaybe :: Event -> (LB.ByteString, Maybe T.Text) 
getHdrBdyMaybe Event {path, headers, body} = (preProcessHeaders headers, 
         TLO.decode $ (LB.fromStrict (T.encodeUtf8 (fromMaybe "" body))))
