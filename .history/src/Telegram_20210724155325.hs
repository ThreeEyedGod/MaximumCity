...
handleUpdate helper Update {message = Just m} = do
    ...
    return $ parseTelegramMessage whatUserTyped helper c
{--
...
parseTelegramMessage :: T.Text -> T.Text -> ChatId -> TelegramClient ()
parseTelegramMessage wut msgToSend c 
      | "/start" `T.isPrefixOf` wut == True = do
                          _rm1 <- sendMessageM $ sendMessageRequest c $ "Hi! I am @MaximumCityBot \nEnter your place name "
      | "/help" `T.isPrefixOf` wut == True = do
                          _rm1 <- sendMessageM $ sendMessageRequest c $ "Enter one of these : Mumbai, Borivali, Maharashtra or your location"
      | "/" `T.isPrefixOf` wut == True = do
                          _rm1 <- sendMessageM $ sendMessageRequest c $ "Hi! I am @MaximumCityBot \nEnter your place name "
      | otherwise       =  do
                          _rm2 <- sendMessageM $ sendMessageRequest c $ helper
    return ()
