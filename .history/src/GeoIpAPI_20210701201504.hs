...
getLatLongforThis town = do 
  ...
  case d of
    ...
    Right geoipstuff -> do
      _d <- _data (geoipstuff :: ForwardGeoData)
      _l <- latitude ( _d :: Locdata)
      return $ Data.ByteString.Char8.pack (show (latitude ((_data (geoipstuff :: ForwardGeoData))) :: Locdata) ++ "," ++ show  (longitude (geoipstuff :: ForwardGeoData)))
