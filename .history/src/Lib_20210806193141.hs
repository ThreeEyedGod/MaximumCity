...
processApiGatewayRequest t tc Event {path, headers, body} context = 
 ...
 -}
processApiGatewayRequest :: T.Text -> Maybe TC -> Event -> Context context -> IO (LambdaResult 'APIGatewayHandlerType)
...
mkRespBodygetUpdate :: Event -> IO (T.Text, Update)
mkRespBodygetUpdate Event {path, headers, body} = do
    case eitherDecode (LB.fromStrict (T.encodeUtf8 (fromMaybe "" body))) of
      Left _ -> do
          responseBody <- getTownNameWeatherFromIp (preProcessHeaders headers)
          pure (responseBody, undefined)
      Right update -> do 
          responseBody <- getTownNameWeatherFromTown (gettheTelegram update)
          pure (responseBody, update)