...
import Weather (getTownNameWeatherFromIp, getTownNameWeatherFromTown, getWeather)
...
preProcessEvent :: Event -> (Maybe PreprocessedHeaders, Maybe Update)
preProcessEvent Event {path, headers, body} = 
    case eitherDecode (LB.fromStrict (T.encodeUtf8 (fromMaybe "" body))) of
      Left _ -> do 
                r <- (preProcessHeaders headers)
                pure (r, Nothing)
      Right evBody -> pure (Nothing, Just evBody)

mkRespBodygetUpdate :: Event -> IO (T.Text, Maybe Update)
mkRespBodygetUpdate Event {path, headers, body} = do
  (h, u) <- preProcessEvent Event {path, headers, body}
  responseBody <- getWeather h u
  pure (responseBody, u)
