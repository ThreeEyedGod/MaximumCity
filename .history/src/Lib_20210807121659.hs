...
mkRespBodygetUpdate :: Event -> IO (T.Text, Maybe Update)
mkRespBodygetUpdate Event {path, headers, body} = do
    (responseBody, update) <-  case eitherDecode (LB.fromStrict (T.encodeUtf8 (fromMaybe "" body))) of
      Left _ -> pure (getTownNameWeatherFromIp (preProcessHeaders headers), Nothing)
      Right evBody -> pure (getTownNameWeatherFromTown (gettheTelegram evBody), Just evBody)
