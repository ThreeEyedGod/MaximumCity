...
handleUpdate :: T.Text -> Update -> Maybe ChatId -> TelegramClient ()
handleUpdate helper Update {message = Just m} Nothing = do
    ...
    if "/start" `T.isPrefixOf` whatUserTyped
      then do
        ...
        h <- PirateWeatherAPI.getWeatherForTown $ T.unpack $ whatUserTyped
        handleUpdate h (Update (text m) (Just c)
        return ()

handleUpdate helper _ c = do
    _rm2 <- sendMessageM $ sendMessageRequest (fromMaybe 1 c) $ helper
    return ()

handleUpdate _ u Nothing =
