...
getTownNameWeatherFromTown1 :: PlaceName -> IO TheWeatherThere
getTownNameWeatherFromTown1 town = do
  let t = Data.ByteString.Char8.unpack $ town 
  weather1 <- PirateWeatherAPI.getWeatherForTown $ t
  case `T.isPrefixOf` weather1 of 
    | "Fail" -> 
  if "Fail" `T.isPrefixOf` weather1
  then do                 
    weather2 <- OpenWeatherAPI.getWeatherForTown $ Data.ByteString.Char8.unpack $ town
...
mkWeatherResponse :: PlaceName -> TheWeatherThere -> TheWeatherThere
mkWeatherResponse pl twt = do
    let tw = (Data.ByteString.Char8.unpack pl ++ " is currently " ++ Data.ByteString.Char8.unpack twt)
    return $ Data.ByteString.Char8.pack tw
