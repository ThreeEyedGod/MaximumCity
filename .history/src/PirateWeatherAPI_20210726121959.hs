            ...
            case maybeHead $ (alerts stuff) of
                Just firstAlert -> do
                  ...
                  {--return $ Data.ByteString.Char8.pack $ (Prelude.unlines [ --unlines sticks in newline after each element
                    ...
                    (catSS "Heads Up: " $ description $ firstAlert ),
                    " "] ++ rightNowWeather) --}
                   return $ Data.ByteString.Char8.pack $ (Prelude.unlines $ getAllalerts (alerts stuff)) ++ rightNowWeather
                Nothing -> do 
...
getAllalerts [] = []
getAllalerts (x : xs) = 
          Prelude.unlines [ --unlines sticks in newline after each element
                              (catSS "ALERT! " $ title $ x ),
