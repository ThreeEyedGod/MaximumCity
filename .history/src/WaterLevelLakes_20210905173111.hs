...
extractDivisionData _ Nothing = Nothing 
...
getWaterLakeLevelForPlace_LiveToday_wrtStorage :: T.Text -> IO (Maybe (Region, PercentLiveStorage))
getWaterLakeLevelForPlace_LiveToday_wrtStorage pl = do
    ...
    extractDivisionData (BSU.fromString nrstRegion) maybe_allDivData
    case maybe_allDivData of
        Nothing -> return Nothing
        Just arrRE ->  do 
            --x <- extractDivisionData (BSU.fromString nrstRegion) $ getAllDivData "All"
            x <- extractDivisionData (BSU.fromString nrstRegion) arrRE
            case x of 
                Just y -> return $ Just ((BSU.fromString nrstRegion) , getPercentLiveToday y)
                Nothing -> return Nothing 
