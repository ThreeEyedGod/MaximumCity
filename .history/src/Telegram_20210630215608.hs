...
handleUpdate helper Update {message = Just m} = do
  ...
  then do
    ...
    return ()
  else do getW whatUserTyped >>= smr c >>= smm where
      getW :: T.Text -> T.Text
      getW wut = return (PirateWeatherAPI.getWeatherForTown $ T.unpack $ wut)
      smr :: ChatId -> T.Text -> SendMessageRequest
      smr cid mes = return (sendMessageRequest cid mes)
      smm :: SendMessageRequest -> TelegramClient MessageResponse
      smm x = return (sendMessageM x)
