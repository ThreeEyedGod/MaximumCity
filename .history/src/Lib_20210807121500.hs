...
mkRespBodygetUpdate :: Event -> IO (T.Text, Maybe Update)
mkRespBodygetUpdate Event {path, headers, body} = do
    (responseBody, maybe update) <-  case eitherDecode (LB.fromStrict (T.encodeUtf8 (fromMaybe "" body))) of
      Left _ -> (getTownNameWeatherFromIp (preProcessHeaders headers), Nothing)
      Right evBody -> (getTownNameWeatherFromTown (gettheTelegram update), Just update)
