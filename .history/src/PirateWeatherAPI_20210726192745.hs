...
jsonURL q v = "https://api.pirateweather.net/forecast/" ++ key ++ "/" ++ c
  "http://api.openweathermap.org/data/2.5/weather?q=" ++ q ++ "&units=metric&appid=e7b9dd9f41c3ac26eae9e94536c8075e&lang=en"
  ...
  if "Fail " `isPrefixOf` c
    then do 
      ...
      case k of
       ...
       Right key ->  do
          let urlCall =  "https://api.pirateweather.net/forecast/" ++ key ++ "/" ++ c
          d <- (eitherDecode <$> (getJSON urlCall)) :: IO (Either String DarkSky)
          case d of
