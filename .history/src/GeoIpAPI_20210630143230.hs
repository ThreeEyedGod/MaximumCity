...
getLatLongforThis town = do 
  d <- (eitherDecode <$> getForwardGeoCodeforThis town) :: IO (Either String GeoIp)
  case d of
    Left e ->  do 
                f <- (eitherDecode <$> getForwardGeoCodeforThis town) :: IO (Either String ForwardGeoCode)
                case f of 
                  Left err -> return $ "Mystery Place" 
                  Right geoForwardCode -> return $ Data.ByteString.Char8.pack (show (latitude geoForwardCode) ++ "," ++ show (longitude geoForwardCode))
