...
getPirateWeatherSettings :: IO (Either String TC)
getPirateWeatherSettings = do
  tk <- getKey "PIRATE_WEATHER_TOKEN"
  case tk of
    Left msg -> trace ("Left " ++ show msg) $ pure $ Left $ "Pirate Weather Token error"
    Right token -> do
      trace ("Right  " ++ show token) $ return $ Right $ token
