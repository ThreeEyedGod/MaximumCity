...
--_helperLivePercent (Just a, Just b) = " % livelake level at " ++ (BSU.toString a) ++ " is " ++ Data.ByteString.Char8.unpack $ TSE.decodeUtf8 $ (percent_Today b)
_helperLivePercent (Just a, Just b) = " % livelake level at " ++ (BSU.toString a) ++ " is " ++ BSU.(percent_Today b)
