...
getTownNameWeatherFromTown town = do
  let (region, lakelevel) <- getWaterLakeLevelForPlace_LiveToday_wrtStorage town
  let wll = fromMaybe "" (Data.ByteString.Char8.unpack $ TSE.decodeUtf8 $ (percent_Today lakelevel))              
