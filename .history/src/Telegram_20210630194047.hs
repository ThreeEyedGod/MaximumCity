...
handleUpdate helper Update {message = Just m} = do
  ...
  case whatUserTyped of
    "/start" -> do
                ...
                return()
    _        -> do
                --weather1 <- PirateWeatherAPI.getWeatherForTown $ Data.ByteString.Char8.unpack $ whatUserTyped
                weather1 <- PirateWeatherAPI.getWeatherForTown $ TS.unpack $ whatUserTyped
                _rm3 <- sendMessageM $ sendMessageRequest c $ weather1
