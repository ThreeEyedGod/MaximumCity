...
_goGetDarkSkyJson ll kee 
  | "Fail:" `isPrefixOf` ll = return $ Left $ _returnStdFail "getWeatherForTown" "getLatLongforThis"
  | isLeft kee = return $ Left _returnStdFail "getWeatherForTown" "getPirateWeatherSettings"
  | otherwise  =  (eitherDecode <$> (getJSON (theURL (fromRight defaultKey kee)) ll)) :: IO (Either String DarkSky)
