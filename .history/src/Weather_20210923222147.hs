...
_handlePirateResponse "Fail" town wll "Fail"  = do
...
_handlePirateResponse weather1 town wll weather1 =  do
...
_getTownNameWeatherFromTown town = do
  (region, lakelevel) <- getWaterLakeLevelForPlace_LiveToday_wrtStorage town
  let wll = _helperLivePercent (region,lakelevel)
  PirateWeatherAPI.getWeatherForTown $ Data.ByteString.Char8.unpack $ town >>= _handlePirateResponse town wll
