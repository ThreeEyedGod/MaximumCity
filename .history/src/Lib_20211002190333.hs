...
mkRespBodygetUpdate Event {path, headers, body} = getHdrBdyMaybe Event {path, headers, body} >>= 
                (\(x , y) -> pure $ ( (getWeather (Just x) (gettheTelegramMaybe y)) , y))

_getAllInputs :: Event -> AllInputs
_getAllInputs Event {path, headers, body} = getHdrBdyMaybe Event {path, headers, body}

_getWeatherThere :: AllInputs -> IO TheWeatherThere
_getWeatherThere allIn = gettheTelegramMaybe $ snd allIn >>= (\x -> getWeather (Just (fst allIn)) x
