...
processApiGatewayRequest t tc Event {path, headers, body} context = 
 ...
 -}
processApiGatewayRequest :: T.Text -> TC -> Event -> Context context -> IO (LambdaResult 'APIGatewayHandlerType)
processApiGatewayRequest t tc Event {path, headers, body} context = do  
  let responseBody  = mkResponseBody Event {path, headers, body}   
  let responseBodyText :: ApiGatewayResponseBody = ApiGatewayResponseBody responseBody
...
mkResponseBody :: Event -> T.Text
mkResponseBody e {path, headers, body} = do
  eventBody <- case eitherDecode (LB.fromStrict (T.encodeUtf8 (fromMaybe "" body))) of
    Left _ -> "No event Body"
    Right canProcessForTelegram -> pure canProcessForTelegram


processTelegramUpdate :: T.Text -> TC -> Update -> IO T.Text
processTelegramUpdate t tc eb = do
    rb <- getTownNameWeatherFromTown (gettheTelegram eb)          
    if not (Prelude.null (T.unpack t)) then 
        return rb
    else 
        runTC tc $ handleUpdate rb eb         
        return rb