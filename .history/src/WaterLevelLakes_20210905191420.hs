                ...
                case s of 
                  "Major" -> 
                    case maybeHead $ (categoryProjectsLinedata (majorMaharashtraStateProjects waterLevel)) of 
                        ...
                        Nothing -> return Nothing -- error "No Region data "
                  "Medium" -> 
                  ...
                  "All" -> 
                    case maybeHead $ (categoryProjectsLinedata (allDamsMaharashtraStateProjects waterLevel)) of 
                        ...
                        Nothing -> return Nothing -- error "No Region data "
                  otherwise -> return Nothing --error "Ouch"
...
getWaterLakeLevelForPlace_LiveToday_wrtStorage pl = do
    ...
    maybe_allDivData <- getAllDivData "All1" 
...
getliveWL ::  T.Text -> IO T.Text
getliveWL town = do
    (region, lakelevel) <- (Just region, Just lakelevel) <- getWaterLakeLevelForPlace_LiveToday_wrtStorage town
    let wll = Data.ByteString.Char8.unpack $ TSE.decodeUtf8 $ (percent_Today lakelevel)              
    let livelWL = (Data.ByteString.Char8.unpack town ++ " is currently " ++ Data.ByteString.Char8.unpack weather2 ++ " % livelake level at " ++ (BSU.toString region) ++ " is " ++ wll )
