...
processApiGatewayRequest t tc Event {path, headers, body} context = 
  do
    responseBody <- case eitherDecode (LB.fromStrict (T.encodeUtf8 (fromMaybe "" body))) of
      Left _ -> responseBody <- do $ getTownNameWeatherFromIp (preProcessHeaders headers)
      Right update -> responseBody <- do $ processTelegramUpdate t tc update  
