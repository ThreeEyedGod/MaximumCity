...
_helperLivePercent _Nothing = " % livelake level Not Available "
_helperLivePercent (Just a,b)
    | (isJust a) && (isJust b) = " % livelake level at " ++ (BSU.toString (Just a)) ++ " is " ++ Data.ByteString.Char8.unpack $ TSE.decodeUtf8 $ (percent_Today (Just b))
