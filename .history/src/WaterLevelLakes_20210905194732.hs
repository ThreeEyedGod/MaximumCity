...
getSpecificProjectSizeDataCategoryProjectsLineData (Just cpjs) = (categoryProjectsLinedata cpjs)
    case maybeHead $ (categoryProjectsLinedata cpjs) of 
        Just isThere ->  Just (categoryProjectsLinedata cpjs)                            
        Nothing ->  Nothing 
...
getAllDivData s = do
     ...
     case w of 
         Left err -> return Nothing
         Right waterLevel -> do
                cpjs <- getSpecificProjectSizeDataCategoryProjects s waterLevel
                ld <- getSpecificProjectSizeDataCategoryProjectsLineData cpjs
     case ld of 
         Nothing -> return Nothing
         Just linedata -> pure linedata 
