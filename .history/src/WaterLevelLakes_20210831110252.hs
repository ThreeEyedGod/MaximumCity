...
getAllDivDataMedium :: Page8Page9 -> IO [RegionEntry]
getAllDivDataMedium p = do
     m <- getAllDivDataMajorWithP8P9 p
     case maybeHead $ (categoryProjectsLinedata (mediumMaharashtraStateProjects p)) of 
                        Just isThere -> pure (categoryProjectsLinedata (mediumMaharashtraStateProjects waterLevel))                            
                        Nothing -> error "No Region data "


getAllDivDataMajorWithP8P9 :: Page8Page9 -> IO [RegionEntry]
getAllDivDataMajorWithP8P9 p = do
    maybeHead $ (categoryProjectsLinedata (majorMaharashtraStateProjects p)) of 
        Just isThere -> pure (categoryProjectsLinedata (majorMaharashtraStateProjects p))                            
        Nothing -> error "No Region data "
...
getWaterLakeLevelForPlace_LiveToday_wrtStorage pl = do
            ...
            x <- extractDivisionData (TSE.encodeUtf8 "Kokan") getAllDivDataMajor
            case x of 
