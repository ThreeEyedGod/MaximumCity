...
_goGetDarkSkyJson :: (IO LatLong , IO (Either String Key) -> IO (Either String DarkSky)
...
_extractWeather :: Either String DarkSky -> Text
_extractWeather dS 
  ...
  | isRight dS = do 
      ...
      case maybeHead $ (alerts d) of
          Just atLeastOneAlert -> Data.ByteString.Char8.pack $ rightNowWeather ++ getAllalerts (alerts d) 
          Nothing -> do 
            case maybeHead $ (dly_data (daily d)) of 
              Just dailyForecast -> Data.ByteString.Char8.pack $ rightNowWeather ++ 
                                      getAllDaysForecast (dly_data (daily d))
              Nothing -> Data.ByteString.Char8.pack $ rightNowWeather


_preProcessWeatherBasis :: String -> (IO LatLong , IO (Either String Key)) 
_preProcessWeatherBasis town = getLatLongforThis town >>= (\x -> (x , pure $ getPirateWeatherSettings))

__getWeatherForTown :: String -> IO Text
__getWeatherForTown town =  _preProcessWeatherBasis town >>= (\z -> _goGetDarkSkyJson z) >>= (\x -> _extractWeather x)
