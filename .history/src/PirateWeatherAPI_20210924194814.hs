...
{- 
_preProcessPirateWeather :: Text -> Either String String -> IO Either String (Text, String)
_preProcessPirateWeather latlong key 
  | "Fail:" `isPrefixOf` latlong = return $ "Fail:_preProcessPirateWeather | getLatLongforThis"
  | isLeft key = return $ "Fail:_preProcessPirateWeather | getPirateWeatherSettings"
  | otherwise = return $ (latlong, fromRight "" key)
 -}
...
getWeatherForTown town = do
  ...
  if "Fail:" `isPrefixOf` c
    then do 
      ...
      _returnStdFail "getWeatherForTown" "getLatLongforThis"
    else do 
      k <- getPirateWeatherSettings -- set this in AWS Env. Variables
      case k of
       Left errMsgString -> _returnStdFail "getWeatherForTown" "getPirateWeatherSettings"
       Right key ->  do
          d <- (eitherDecode <$> (getJSON (theURL key) c)) :: IO (Either String DarkSky)
          case d of
            Left e ->  _returnStdFail "getWeatherForTown" "eitherDecode" <> pack e
              return $ "Fail:getWeatherForTown | eitherDecode key " <> pack e
