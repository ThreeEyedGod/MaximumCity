...
toTuple :: [RegionEntry] -> Maybe [(DB.ByteString, RegionWaterData)]
toTuple [] = Nothing
toTuple x:xs = Just $ cons (revenueRegion x, waterData x) (toTuple xs

extractMyData :: DB.ByteString -> [RegionEntry] -> Maybe (DB.ByteString, RegionWaterData)
extractMyData _ []  = Nothing
--extractMyData div xs = 