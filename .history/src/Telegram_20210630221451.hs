...
handleUpdate helper Update {message = Just m} = do
  ...
  then do
    ...
    _rm2 <- sendMessageM $ sendMessageRequest c $ weather $ whatUserTyped
    return ()

weather :: T.Text -> IO T.Text
weather inTown =
    PirateWeatherAPI.getWeatherForTown Data.ByteString.Char8.pack $ inTown
