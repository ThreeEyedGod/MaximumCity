...
_preProcessWeatherBasis town = getPirateWeatherSettings >>= (\a -> (a , getLatLongforThis town) >>= (\(x , y) -> pure $ (x :: LatLong , getPirateWeatyherSettings)) 

_getWeatherForTown :: String -> IO Text
_getWeatherForTown town =  _preProcessWeatherBasis town >>= (\z -> _goGetDarkSkyJson z) >>= (\x -> _extractWeather x)
