...
import qualfied Data.List as DL
...
findNearestRegionToPlace :: String -> IO Region
findNearestRegionToPlace pl = do
    ...
    let mi = minimum filter $ snd di
    let rg = foldl (\(x , y) -> (x , if y eq mi then y else 0)) regionLatLong
    pure $ rg
