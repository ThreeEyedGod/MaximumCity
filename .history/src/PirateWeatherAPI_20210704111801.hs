...
getWeatherForTown town = do
  ...
  case d of
    Left e ->  return $ "Fail"
    Right stuff ->
        return $ Data.ByteString.Char8.pack $ (catSF "Current Tgemperature" (temperature (currently stuff)))
      show (temperature (currently stuff)) ++ " F" ++
      show (summary (currently stuff) ++  (catSF  "Chance of rain " (100 * precipProbability (currently stuff)))
       ++ 
