...
getLatLongforThis town = do 
  d <- (eitherDecode <$> getForwardGeoCodeforThis town) :: IO (Either String Loc)
  case d of
    Left e ->  do 
    ...
    Right geoipstuff -> return $ Data.ByteString.Char8.pack (show (latitude (locdata geoipstuff)) ++ "," ++ show  (longitude (geoipstuff.locdata :: Locdata)))
