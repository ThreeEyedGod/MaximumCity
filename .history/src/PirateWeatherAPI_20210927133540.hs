...
_extractWeather dS 
  | isLeft dS = return $ unpack $ _returnStdFail "getWeatherForTown" "eitherDecode DarkSky"
  | isRight dS = do 
      let Right d = dS
...
__getWeatherForTown town = getLatLongforThis town >>= (\x -> (x, getPirateWeatherSettings)) >>= (\(y , z) -> _goGetDarkSkyJson y z) >>= (\x -> _extractWeather x)
