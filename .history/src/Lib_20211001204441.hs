...
processApiGatewayRequest tc Event {path, headers, body} context = mkRespBodygetUpdate Event {path, headers, body}  >>= (\x -> runTC tc $ _handleUpdate responseBody (snd x)) >> pure $ formAPIGatewayResult 200 (fst x)          
