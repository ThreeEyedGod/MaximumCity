...
{- extractDivisionData :: DB.ByteString -> [RegionEntry] -> (DB.ByteString, RegionWaterData)
extractDivisionData division xs =  filter (\x -> (fst x) == division) $ toTuple xs  -}

extractDivisionData :: DB.ByteString -> [RegionEntry] -> Maybe RegionWaterData
extractDivisionData division xs =  lookup division (toTuple xs)