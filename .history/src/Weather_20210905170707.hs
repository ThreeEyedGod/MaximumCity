...
getTownNameWeatherFromTown town = do
  ...
  then do                 
    ...
    let tw = (Data.ByteString.Char8.unpack town ++ " is currently " ++ Data.ByteString.Char8.unpack weather1 ++ " % livelake level at " ++ (BSU.toString region) ++ " is " ++ (Data.ByteString.Char8.unpack $ TSE.decodeUtf8 $ (percent_Today lakelevel)) )
    return $ Data.ByteString.Char8.pack tw
