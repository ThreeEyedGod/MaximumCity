...
badEnv3 :: EnvError -> IOException -> IO (Either EnvError String)
badEnv3 err = const $ return $ Left err

getEnv3 :: String -> EnvError -> IO (Either EnvError String)
getEnv3 env err = handle (badEnv env) $ Right <$> (getEnv env)
