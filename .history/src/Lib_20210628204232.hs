...
handler :: TC -> Event -> Context -> IO (Either String Lib.Response)
handler tc Event {path, headers, body} context 
    | preProcessBody (fromMaybe "" body) == "Not Telegram" = do
        let responseHeaders <- (object ["Access-Control-Allow-Headers" .= ("*" :: String), "Content-Type" .= ("application/json" :: String), "Access-Control-Allow-Origin" .= ("*" :: String), "Access-Control-Allow-Methods" .= ("POST,GET,OPTIONS" :: String)])  
        pure $ Right $ Lib.Response 200 responseHeaders "no telegram" False

    prcssdHeaders <- preProcessHeaders headers -- gives headers
    case prcssdBody of 
          "Not Telegram" -> pure $ Right $ Lib.Response 200 responseHeaders "Not Telegram" False
          _              -> (kText prcssdHeaders (fromMaybe "" body))
