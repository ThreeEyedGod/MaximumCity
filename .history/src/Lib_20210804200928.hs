...
processApiGatewayRequest :: String -> TC -> Event -> Context context -> IO (LambdaResult 'APIGatewayHandlerType)
processApiGatewayRequest s tc Event {path, headers, body} context = 
  do
    case eitherDecode (LB.fromStrict (T.encodeUtf8 (fromMaybe "" body))) of
      Left _ -> do
        responseBody <- getTownNameWeatherFromIp (preProcessHeaders headers)
...
processTelegramRequest :: String -> TC -> T.Text -> Update -> TelegramClient () -> IO ()
processTelegramRequest s (token, manager) act respB up 
    | not (Prelude.null (unpack s)) = return 
    | otherwise = runTC tc $ handleUpdate responseBody update 