...
_preProcessWeatherBasis :: String -> (LatLong , IO Either String Key) 
_preProcessWeatherBasis town = getLatLongforThis town >>= (\x -> (x , pure $ getPirateWeatherSettings))

__getWeatherForTown :: String -> IO Text
__getWeatherForTown town =  _preProcessWeatherBasis town >>= (\(y , z) -> _goGetDarkSkyJson y z) >>= (\x -> _extractWeather x)
