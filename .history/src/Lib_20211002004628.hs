...
mkRespBodygetUpdate Event {path, headers, body} = getHdrBdyMaybe Event {path, headers, body} >>= (\a -> 
    getWeather (Just (fst a)) (gettheTelegramMaybe (snd a))) >>=  (\b -> pure (responseBody, snd a)
