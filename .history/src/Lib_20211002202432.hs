...
type AllInputs  = (ResponseBody, Maybe Update) 
...
processApiGatewayRequest :: Maybe TC -> Event -> Context context -> IO (LambdaResult 'APIGatewayHandlerType)
processApiGatewayRequest tc Event {path, headers, body} context = _processApiGatewayRequest tc Event {path, headers, body} context

_callTelegramClient :: runTC :: Maybe TC -> AllInputs -> IO ()
_callTelegramClient tc allin = runTC tc $ _handleUpdate 

_processApiGatewayRequest :: Maybe TC -> Event -> Context context -> IO (LambdaResult 'APIGatewayHandlerType)
_processApiGatewayRequest tc Event {path, headers, body} context = mkRespBodygetUpdate Event {path, headers, body} >>= 
  \(responseBody, update) -> (pure $ formAPIGatewayResult 200 responseBody
        where
          runTC tc $ _handleUpdate responseBody update)
