...
getMyKey2 :: String -> IO (Either EnvError String)
getMyKey2 k = runExceptT $ do
  ExceptT $ handle (mapIOError2 ex) $ Right <$> (getEnv env)

mapIOError2 :: SomeException -> IO a -> ExceptT EnvError IO a
mapIOError2 env msg = ExceptT . fmap (first (mapError env)) . tryIOError
  where
    mapError e err
