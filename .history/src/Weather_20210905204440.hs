...
_helperLivePercent :: (Maybe Region, Maybe PercentLiveStorage) -> T.Text
_helperLivePercent (a,b)
    | a && b = " % livelake level at " ++ (BSU.toString a) ++ " is " ++ Data.ByteString.Char8.unpack $ TSE.decodeUtf8 $ (percent_Today b)
    | otherwise = " % livelake level Not Available " ++ (BSU.toString a) ++ " is " ++ Data.ByteString.Char8.unpack $ TSE.decodeUtf8 $ (percent_Today b)
...
getTownNameWeatherFromTown town = do
  ...
  case (isJust lakelevel) && (isJust region) of 
    | True = 
        let wll = Data.ByteString.Char8.unpack $ TSE.decodeUtf8 $ (percent_Today lakelevel)
        let tw_ll = " % livelake level at " ++ (BSU.toString region) ++ " is " ++ wll
    | otherwise =   
        let tw_ll = ""
  ...
  then do                 
    ...
    let tw = (Data.ByteString.Char8.unpack town ++ " is currently " ++ Data.ByteString.Char8.unpack weather2 ++ " % livelake level at " ++ (BSU.toString region) ++ " is " ++ wll )
