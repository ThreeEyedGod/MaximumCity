...
getLatLongforThis town = do 
  ...
  case d of
    ...
    Right geoipstuff -> 
      g <- (eitherDecode <$> getForwardGeoCodeforThis town) :: IO (Either String ForwardGeoData)
      return $ Data.ByteString.Char8.pack (show (_data (geoipstuff :: ForwardGeoData)) ++ "," ++ show  (_data (geoipstuff :: ForwardGeoData)))
