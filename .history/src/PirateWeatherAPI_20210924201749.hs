...
_goGetDarkSkyJson ll kee 
  | "Fail:" `isPrefixOf` ll = Left $ _returnStdFail "getWeatherForTown" "getLatLongforThis"
  | isLeft kee = Left _returnStdFail "getWeatherForTown" "getPirateWeatherSettings"
  | otherwise  =  return Right $ (eitherDecode <$> (getJSON (theURL kee) ll)) :: IO (Either String DarkSky)
