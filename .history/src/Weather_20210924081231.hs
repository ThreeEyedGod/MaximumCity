...
_handlePirateResponse weather1 town wll 
   | "Fail:" `T.isPrefixOf` weather1 = (OpenWeatherAPI.getWeatherForTown $ Data.ByteString.Char8.unpack $ town) >>= (\weather2 -> _mkWeatherThere town weather2 wll)
   | otherwise =  _mkWeatherThere town weather1 wll

__handlePirateResponse :: PlaceName -> WaterLevel -> IO TheWeatherThere
__handlePirateResponse  t wl = _handlePirateResponse

_getTownNameWeatherFromTown :: PlaceName -> IO TheWeatherThere
_getTownNameWeatherFromTown town = (PirateWeatherAPI.getWeatherForTown $ Data.ByteString.Char8.unpack town) >>= 
  (\w -> (getWaterLakeLevelForPlace_LiveToday_wrtStorage town >>=  _helperLivePercent) >>= (\wl w -> _handlePirateResponse w town wl))
