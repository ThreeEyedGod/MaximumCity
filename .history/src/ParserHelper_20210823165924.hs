...
_takeWhile :: Word8 -> Parser ByteString
_takeWhile w8 = takeWhile (\c -> c /= w8 )
...
deleteLaterParser :: Parser DeleteLater
deleteLaterParser = do 
    endOfLine
    a <- textTillaDigit
    b <- decimal
    space
    return $ DeleteLater a b
...
parseTestCPP :: ByteString -> Either String 
parseTestCPP = parseOnly categoryProjectsParser

parseTestTextTillaDigit :: ByteString -> IO ()
parseTestTextTillaDigit = parseTest textTillaDigit

parseTestDeleteLater :: ByteString -> IO ()
parseTestDeleteLater = parseTest deleteLaterParser