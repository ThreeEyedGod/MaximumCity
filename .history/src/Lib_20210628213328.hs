...
handler :: TC -> Event -> Context -> IO (Either String Lib.Response)
handler tc Event {path, headers, body} context = do
  prossdBody <- preProcessBody (fromMaybe "" body)
     case preProcessBody (fromMaybe "" body) of
        "Not Telegram" -> pure $ Right $ Lib.Response 200 (object ["Access-Control-Allow-Headers" .= ("*" :: String), "Content-Type" .= ("application/json" :: String), "Access-Control-Allow-Origin" .= ("*" :: String), "Access-Control-Allow-Methods" .= ("POST,GET,OPTIONS" :: String)]) "Not Telegram" False
        _              -> pure $ Right $ Lib.Response 200 (object ["Access-Control-Allow-Headers" .= ("*" :: String), "Content-Type" .= ("application/json" :: String), "Access-Control-Allow-Origin" .= ("*" :: String), "Access-Control-Allow-Methods" .= ("POST,GET,OPTIONS" :: String)]) "Telegram" False
    --runTC tc $ handleUpdate responseBody update