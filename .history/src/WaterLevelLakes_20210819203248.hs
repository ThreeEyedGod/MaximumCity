...
colHeaderParser :: Parser RevenueRegionHeader
colHeaderParser = do
    eachRevenueRegionHeaders =  manyTill textGrabberDiscard endOfLine
    srNo <-  textGrabberDiscard
    char '\n'
    revenueRegion <- textGrabberDiscard
    char '\n'
    noDams <- textGrabberDiscard
    char '\n'
    designedStorage <- textGrabberDiscard
    char '\n'
    todaysLiveStorage <- textGrabberDiscard
    char '\n'
    percentageLiveStorageWRTDesignedLiveStorage <- textGrabberDiscard
    char '\n'
    dead_DesignedStorage <- textGrabberDiscard
    char '\n'
    live_DesignedStorage <- textGrabberDiscard
    char '\n'
    gross_DesignedStorage <- textGrabberDiscard
    char '\n'
    live_TodayLiveStorage <- textGrabberDiscard
    char '\n'
    gross_TodayLiveStorage <- textGrabberDiscard
    char '\n'
    percent_Today_LiveStorage <- textGrabberDiscard
    char '\n'
    percent_SameDateLastYear_LiveStorage:: T.Text
    return $ RevenueRegionHeader srNo revenueRegion noDams designedStorage todaysLiveStorage percent_LiveStorage_WRT_liveDesignedStorage
                            dead_DesignedStorage live_DesignedStorage gross_DesignedStorage live_TodayLiveStorage
                                gross_TodayLiveStorage percent_Today_LiveStorage percent_SameDateLastYear_LiveStorage

colNumbersParser :: Parser ColumnNos
colNumbersParser = do 
    one <- decimal
    skipSpaces
    return $ ColumnNos one 
...
type eachRevenueRegionHeaders = T.Text
