...
parserightNowWeather :: DarkSkyDataPoint -> IO Text 
parserightNowWeather dsdp = Data.ByteString.Char8.pack $ Prelude.unlines [ --unlines sticks in newline after each element
                    (catSF "at Celsius " $ fahrenheitToCelsius (temperature dssp)),
                    (catSS "" (summary dsdp)),
...
getWeatherForTown town = do
  ...
  case d of
    ...
    Right stuff -> do
        let rightNowWeather = parserightNowWeather (currently stuff) 
            ...
            Nothing -> do 
              case maybeHead $ (daily stuff) of 
                Just dailyForecast -> return $ Data.ByteString.Char8.pack $ Prelude.unlines [ --unlines sticks in newline after each element
                      (catSS "Tomorrow! " $ _summary $ dailyForecast ), " "]
                Nothing -> 
                  return $ Data.ByteString.Char8.pack $ Prelude.unlines [ --unlines sticks in newline after each element
