...
handleUpdate helper (Just Update {message = Just m}) = do
    ...
    let hlpAsk = ("/start" `T.isPrefixOf` whatUserTyped) || ("?" `T.isPrefixOf` whatUserTyped) || ("/Help" `T.isPrefixOf` whatUserTyped) || ("Help" `T.isPrefixOf` whatUserTyped) :: preProcessBodytoGetTelegram
