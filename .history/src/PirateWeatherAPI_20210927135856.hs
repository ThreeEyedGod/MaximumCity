...
_preProcessWeatherBasis t = getLatLongforThis ttown >>= (\x -> (x , (\a -> getPirateWeatherSettings)))

__getWeatherForTown :: String -> IO Text
__getWeatherForTown town =  >>= (\(y , z) -> _goGetDarkSkyJson y z) >>= (\x -> _extractWeather x)
