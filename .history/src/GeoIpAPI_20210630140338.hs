...
data dataFCDatasets  = dataFCDatasets
...
getLatLongforThis town = do 
  ...
  case d of
    Left e ->  do 
                f <- (eitherDecode <$> getForwardGeoCodeforThis town) :: IO (Either String ForwardGeoCode)
                case f of 
                  Left err -> return $ "Mystery Place" 
                  Right geoForwardCode -> return $ Data.ByteString.Char8.pack (show (latitude results datasets geoForwardCode) ++ "," ++ show (longitude results datasets geoForwardCode))
