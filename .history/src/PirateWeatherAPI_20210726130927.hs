            ...
            case maybeHead $ (alerts stuff) of
                Just atLeastOneAlert -> do
                      return $ Data.ByteString.Char8.pack $ 
                            ...
                            ++ rightNowWeather
                Nothing -> do 
                  case maybeHead $ (__data (daily stuff)) of 
                    Just dailyForecast -> do
                      return $ Data.ByteString.Char8.pack $ 
                            getAllDaysForecast (__data (daily stuff)) 
                            ++ rightNowWeather
                    Nothing -> 
...
getAllDaysForecast (x : xs) = 
          Prelude.unlines [   (catSI "Sunset at " $ sunsetTime $ x ),
                              (catSF "Moon Phase: " $ moonPhase $ x ),
                              " "] 
