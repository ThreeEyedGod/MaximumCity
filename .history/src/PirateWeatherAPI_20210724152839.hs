  ...
  case d of
    ...
    Right stuff ->
        return $ Data.ByteString.Char8.pack $ Prelude.unlines [ --unlines sticks in newline after each element
          ...
          (catSF  "Visibility " (visibility (currently stuff))), 
          (catSF  "Ozone " (ozone (currently stuff))), 
          (catSI  "Nearest Storm Distance " (nearestStormDistance (currently stuff))), 
          (catSI  "Nearest Storm Bearing " (ozone (currently stuff))), 
          " "]
            nearestStormDistance:: Int, 
            nearestStormBearing:: Int, 
