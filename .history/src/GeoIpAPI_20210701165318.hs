...
instance FromJSON ForwardGeoData where
  parseJSON = genericParseJSON defaultOptions {
    fieldLabelModifier = Prelude.drop 0
  }
...
getLatLongfromPosStack :: String -> Either IO Either (JSONPath, String) ForwardGeoData
getLatLongfromPosStack town = do
  d <- (eitherDecodeWith jsonEOF ifromJSON <$> getForwardGeoCodeforThis town) :: IO (Either (JSONPath, String) ForwardGeoData)
  case d of
    Left e -> return $ e
    Right geoipstuff -> return $ Data.ByteString.Char8.pack (show (_data (geoipstuff :: ForwardGeoData)) ++ "," ++ show (_data (geoipstuff :: ForwardGeoData)))
