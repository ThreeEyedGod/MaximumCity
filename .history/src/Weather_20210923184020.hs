...
getTownNameWeatherFromIp headers = do
  ...
  if "Fail" `T.isPrefixOf` town
    then do
      return $ "Fail:getTownNameWeatherFromIp | town"
    else do
      getTownNameWeatherFromTown town

_getTownNameWeatherFromIp :: IO PlaceName -> IO TheWeatherThere
_getTownNameWeatherFromIp town
  | "Fail" `T.isPrefixOf` town = return $ "Fail:getTownNameWeatherFromIp | town"
  | otherwise = getTownNameWeatherFromTown town
...
_handlePirateResponse :: T.Text -> PlaceName -> String -> IO TheWeatherThere
...
getWeather (Just p) Nothing = _getTownNameWeatherFromIp $ extractXForwardedForHeader p
