...
getWeatherForTown town = do
  ...
  case d of
    Left e ->  return $ "Fail"
    Right stuff -> do
        let x = (myStringParms (show (temperature (currently stuff))) "F")  
          ++ (myStringParms (show (summary (currently stuff)) "")
        return $ Data.ByteString.Char8.pack $ x
...
myStringParms :: String -> String -> String
myStringParms core sufix = core ++ sufix ++ "\n"

(myStringParms (show (temperature (currently stuff))) "F")  
          ++ (myStringParms (show (summary (currently stuff)) "")