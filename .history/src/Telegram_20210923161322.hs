...
selectMsg :: Bool -> T.Text -> T.Text
selectMsg true _ = "Hi! I am @MaximumCityBot \nEnter your place name \nEnter For ex: \nMumbai, \nPune \nMaharashtra\n Bhivandi\n "
selectMsg false rMsg = rMsg

_handleUpdate :: T.Text -> Maybe Update -> TelegramClient ()
_handleUpdate helper (Just Update {message = Just m}) 
  | hlpAsk == false  = _pushTelegramMsg m c
  | otherwise       = _pushTelegramMsg 
    where
