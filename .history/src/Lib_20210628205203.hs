...
handler tc Event {path, headers, body} context = do
     case preProcessBody (fromMaybe "" body) of
        ...
        _              -> pure $ Right $ Lib.Response 200 (object ["Access-Control-Allow-Headers" .= ("*" :: String), "Content-Type" .= ("application/json" :: String), "Access-Control-Allow-Origin" .= ("*" :: String), "Access-Control-Allow-Methods" .= ("POST,GET,OPTIONS" :: String)]) "Not Telegram" False