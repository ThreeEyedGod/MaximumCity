...
getWeatherForTown town = do
  ...
  case d of
    ...
    Right stuff -> do
        case decode (alerts stuff) :: Maybe [DarkSkyAlert] of
            Just someAlerts -> print $ title $ (Prelude.take 1 someAlerts)!!0
            Nothing -> Prelude.putStrLn "No Alerts"
        ...
        return $ Data.ByteString.Char8.pack $ Prelude.unlines [ --unlines sticks in newline after each element
          ...
          " ",
          --(catSS  "Alert: Headline " (title $ (Prelude.take 1 $ alerts stuff)))), 
          --(catSS  "Alert: What " (description (alerts stuff))), 
          --(catSS  "Alert: Severity " (severity (alerts stuff))),         
          " "]
