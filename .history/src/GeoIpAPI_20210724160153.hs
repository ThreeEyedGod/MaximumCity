...
getForwardGeoCodeforThis town = do
  k <- getEnv "API_POSITIONSTACK_KEY" -- set this in AWS Env. Variables

  key <- getKey "API_POSITIONSTACK_KEY"
  case key of
    Left msg -> trace ("Left " ++ show msg) $ pure $ Left $ "API_POSITIONSTACK_KEY error"
    Right token -> do
