...
getTownNameWeatherFromTown town = do
  (Just region, Just lakelevel) <- getWaterLakeLevelForPlace_LiveToday_wrtStorage town
  ...
  then do                 
    ...
    let tw = (Data.ByteString.Char8.unpack town ++ " is currently " ++ Data.ByteString.Char8.unpack weather2 ++ " % livelake level at " ++ (BSU.toString (fromMaybe "" region)) ++ " is " ++ wll )
    return $ Data.ByteString.Char8.pack tw
  else do  
    let tw = (Data.ByteString.Char8.unpack town ++ " is currently " ++ Data.ByteString.Char8.unpack weather1 ++ " % livelake level at " ++ (BSU.toString (fromMaybe "" region)) ++ " is " ++ wll )
    return $ Data.ByteString.Char8.pack tw
