...
items_sepByComma s                 =  case dropWhile {-partain:Char.-}isPunctuation s of
                                "" -> []
                                s' -> w : items_sepByComma s''
                                      where (w, s'') =
                                             Prelude.break {-partain:Char.-}isPunctuation s'
...
extLatLon str =  (x , y) where
    ...
    (x, y) = (degree (read (head z) :: Rational), degree (read (last z) :: Rational))
...
regionLatLong = [("Thane", mkGeo (degree 19, degree 72)), ("Amravati", mkGeo (degree 23, degree 75))]
