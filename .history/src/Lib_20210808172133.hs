...
prepHdrBdy Event {path, headers, body} = 
  do   
    let bdy = hush . eitherDecode (LB.fromStrict (T.encodeUtf8 (fromMaybe "" body)))
    pure (h, Just (gettheTelegram $ hush . 
    case eitherDecode (LB.fromStrict (T.encodeUtf8 (fromMaybe "" body))) of
        Left _ -> pure (h, Nothing)
        Right update -> pure (h, Just (gettheTelegram update))
  where 
      h = preProcessHeaders headers 
      b = gettheTelegramMaybe bdy
