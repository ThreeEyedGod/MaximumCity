...
libHProperty2 :: Spec
libHProperty2 = do
  modifyMaxSuccess (const 1) $ prop "set new env Key-and-get-back "
            (forAll genKeys prop_thereAndBackAgain)
...
prop_uber :: Property
prop_uber = monadicIO $ do
  keyvalpair <- run $ genKeys
  prop_thereAndBackAgain keyvalpair
