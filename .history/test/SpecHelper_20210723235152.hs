...
prop_checkaNullKeyForExactError = monadicIO $ do -- expect this function to fail!
              ...
              shouldBeErr <- run $ handleLeft z -- IO extract from our getKey. Hopefully Left something 
              assert $ (shouldBeErr == EmptyKeyError "Environment key not given ") -- Handle should return exact error
...
prop_checkForOccurenceofSomeIOError = monadicIO $ do -- expect this function to fail!
              z <- run $ getKey "=" -- IO attempt to get key value for bad bad equal using our function
              shouldBeErr <- run $ handleLeft z -- IO extract from our getKey. Hopefully Left something 
              assert $ (shouldBeErr == SomeIOError IOError) -- Handle should return exact error
