...
-- Because getKey are IO, it has to be monadicIO - this test case is not working
...
prop_checkForOccurenceofSomeIOError = monadicIO $ do -- expect this function to fail!
              ...
              shouldBeErr <- run $ handleLeft z -- IO extract from our getKey. Hopefully Left something 
              assert $ ( not (shouldBeErr == EmptyKeyError "Environment key not given ") && not (shouldBeErr == MissingEnvError "") )
