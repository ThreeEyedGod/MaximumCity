...
-- Note: Bind (<-) operations must be run $ SomeIOFunction. Others may be bound without using 
...
propIO2 :: Show a => IO (Gen a) -> (a -> IO Bool) -> Property
...
prop_thereAndBackAgain (k,v) = monadicIO $ do
              run $ SE.setEnv k v
              z <- run $ getKey k
              cleanSetVals <- run $ SE.unsetEnv k
              noErr <- run $ handle z
              assert $ (noErr == v) 
