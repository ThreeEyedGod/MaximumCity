...
libHProperty1 :: Spec
libHProperty1 = do
        modifyMaxSuccess (const 10) $ prop "Test: Always isLeft" 
            prop_gk1
...
prop_thereAndBackAgain = monadicIO $ do
              ...
              j <- pick i -- not an IO
              run $ SE.setEnv (fst j) (snd j) -- a pair-value hence fst and snd
              z <- run $ getKey (fst j) -- get new key value using our function
              cleanSetVals <- run $ SE.unsetEnv (fst j) -- remove. We are done 
              noErr <- run $ handle z -- extract the just set value
...
prop_checkExistingKeys = monadicIO $ do
              j <- run $ SE.getEnvironment -- IO hence needs run $ 
              k <- Prelude.map fst j
              l <- pick k
              z <- run $ getKey l -- IO: get key value using our function
              noErr <- run $ handle z -- extract our value
              assert $ (noErr == snd (Prelude.filter (k == fst j) j))  -- compare with existing value

-- an getEnvironment operation results in a list of strings currently in the environment. Hence marked 
-- as IO String
