...
libHProperty4 :: Spec
libHProperty4 = do
        modifyMaxSuccess (const 10) $ prop "Test: check a Non-existing Key "
...
handleLeft :: Either a b -> IO b
handleLeft (Left err)  = Prelude.putStrLn "exception!" >> exitWith (ExitFailure 1)
handleLeft (Right x) = return x
