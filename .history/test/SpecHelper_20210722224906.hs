...
libHProperty2 :: Spec
libHProperty2 = do
  modifyMaxSuccess (const 10) $ prop "set new env Key-and-get-back "
...
genKeys :: IO (String, String)
genKeys = do
              avoidSet <- existingKeys
              (k , v) <- genNewKeys avoidSet
              return (k , v)

genNewKeys :: [String] -> Gen (String, String)mak
genNewKeys xs =  ( 
                  (arbitrary :: Gen String) `suchThat` ((\s -> validKey s) && (s `DL.notElem` xs)), 
                  (arbitrary :: Gen String) `suchThat` (\s -> validValue s) 
                 )
...
--}
{--
prop_keyisGood :: String -> Property
prop_keyisGood k = monadicIO $ do 
    z <- run $ getKey k
    noErr <- run $ handle z
...
prop_checkExistingKeys = monadicIO $ do
  genKeySet <- run $ existingKeys
  forAllM (elements genKeySet) (\k -> (prop_keyisGood k) )
--}
