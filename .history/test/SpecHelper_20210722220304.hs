...
genNewKeys :: [String] -> Gen String
genNewKeys xs =  (arbitrary :: Gen String) `suchThat` ((\s -> validKey s) && (any `notElem` xs))


prop_thereAndBackAgain :: Property
prop_thereAndBackAgain = validKey k && validValue v ==> monadicIO $ do
              run $ SE.setEnv k v
              z <- run $ getKey k
              noErr <- run $ handle z
              assert $ (noErr == v) 
