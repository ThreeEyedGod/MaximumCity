...
prop_top :: Gen (String , String) -> (Gen (String, String) -> IO Bool) -> Property
prop_top kvp boolfn = monadicIO $ do
  v <- genKeys
  b <- run $ prop_thereAndBackAgain_IOBool v
  assert b

prop_thereAndBackAgain_IOBool :: (String, String) -> Bool
prop_thereAndBackAgain_IOBool (k,v) = monadicIO $ do
              run $ SE.setEnv k v
              z <- run $ getKey k
