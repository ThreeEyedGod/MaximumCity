...
prop_gk2 n = (n > 5) ==> monadicIO test where 
    test = do
        str_set = genStringsConst n
        str_set_res <- run $ Prelude.map getKey $ genStringsConst n
        traceShow ("gk2: str_set ", n) $ assert $ Prelude.foldl (&&) True (Prelude.map DE.isLeft str_set_res) == True
