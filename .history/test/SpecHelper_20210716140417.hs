...
genStringsConst :: Int -> IO Gen [String]
...
prop_gk2 :: [String] -> Property
prop_gk2 str_set = (not $ Prelude.null str_set) ==> monadicIO test where 
    test = do
        res <- run $ Prelude.map getKey $ str_set
        res2 <- run $ Prelude.map DE.isLeft res
        traceShow ("gk2: ", str_set) $ assert $ Prelude.foldl (&&) True res2
