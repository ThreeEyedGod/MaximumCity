...
genStringsConst = choose ("ABCDEFG", "ZZZZZZZ")
...
assertProp :: String -> Bool
assertProp forkey = not (Prelude.null forkey) ==> monadicIO test where 
    test = do
        x <- run $ getKey forkey
        traceShow ("gk1: forkey x ", (forkey, x)) $ DE.isLeft x

prop_gk2 :: Property
prop_gk2 = forAll genStringsConst (\x -> assertProp x)
