...
prop_thereAndBackAgain k v = validKey k && validValue v && viableKey k ==> monadicIO $ do
    run $ SE.setEnv k v
    z <- run $ getKey k
    noErr <- run $ handle z
    assert $ noErr == v
...
viableKey :: IO String -> Bool
viableKey k = monadicIO $ do 
  x <- run $ nonExistingKey k 
  return x

nonExistingKey :: String -> IO Bool
nonExistingKey k = do 
  x <- existingKeys 
