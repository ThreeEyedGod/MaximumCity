...
prop_setAndBackAgain k v =
  validKey k && validValue v ==> monadicIO $ do
    run $ SE.setEnv k v
    noErr <- run $ handle (getKey k)
    assert $ noErr == v

validKey k = (not . Prelude.null) k && '\NUL' `Prelude.notElem` k && '=' `Prelude.notElem` k && k $ (not . Prelude.elem) (Prelude.filter . fst . SE.getEnvironment)
validValue v = (not . Prelude.null) v && '\NUL' `Prelude.notElem` v

handle :: IO Either a b -> IO b
