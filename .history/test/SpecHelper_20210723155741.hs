...
keyvalPairs :: Gen (String, String)
keyvalPairs

prop_top :: Gen (String , String) -> ((String, String) -> IO Bool) -> Property
prop_top kvp boolfn = monadicIO $ do
  v <- genKeys
  b <- run $ boolfn v
  assert b

prop_thereAndBackAgain_IOBool :: (String, String) -> IO Bool
prop_thereAndBackAgain_IOBool (k,v) = do
              SE.setEnv k v
              z <- getKey k
              zplus <- SE.unsetEnv k
              noErr <- handle z
