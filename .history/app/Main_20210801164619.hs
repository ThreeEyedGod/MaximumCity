...
main =  do
      ...
      where
            run :: String -> LambdaOptions -> IO (Either (ApiGatewayResponse Text) (ApiGatewayResponse Text))

gatewayHandlerCallback :: RunCallback HandlerType (LambdaOptions context)
gatewayHandlerCallback ht context = 
