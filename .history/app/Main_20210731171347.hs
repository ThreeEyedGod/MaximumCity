...
main = do 
  runLambda (pure()) (run defaultDispatcherOptions)
  where
    run ::  LambdaOptions context -> IO (Either (LambdaError handlerType) (LambdaResult handlerType))
    run opts  = do
      let result = pure $ Right $ H.Response 200 [("Access-Control-Allow-Headers" .= ("*" :: String))]] "OK" False
      where
      ...
      either (pure . Left . LambdaError) (pure . Right . LambdaResult) result
