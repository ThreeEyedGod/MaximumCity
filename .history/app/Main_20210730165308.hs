...
gatewayHandler :: ApiGatewayRequest request -> Context context -> IO (Either (ApiGatewayResponse (ApiGatewayResponse response))
gatewayHandler request context = do
    ...
    responseBody <- "ok"
    response <- ApiGatewayResponse 200 responseHeaders responseBody False
    pure $ Right $ response
