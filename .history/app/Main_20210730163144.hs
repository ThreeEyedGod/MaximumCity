...
determineResponse :: Maybe String -> apiGatewayRequestHeaders (ApiGatewayRequest) -> Text
...
gatewayHandler request context = do
    ...
    apiGatewayResponseHeaders <- (object ["Access-Control-Allow-Headers" .= ("*" :: String), "Content-Type" .= ("application/json" :: String), "Access-Control-Allow-Origin" .= ("*" :: String), "Access-Control-Allow-Methods" .= ("POST,GET,OPTIONS" :: String)])
    r <- getTelegramSettings
    case r of
      Left msg ->   "Fail : No Telegram Setting"
      Right tc ->   runTC tc $ handleUpdate responseBody telegram 
    pure $ Right $ ApiGatewayResponse 200 apiGatewayResponseHeaders responseBody False
