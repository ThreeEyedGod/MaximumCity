...
main = do 
  runLambda (pure()) (run defaultDispatcherOptions mToIO $ fmap snd . flip runStateT HM.empty . runHandlersM $ addAPIGatewayHandler "api-gateway" gatewayHandler )
  where
    --run ::  LambdaOptions context -> IO (Either (LambdaError handlerType) (LambdaResult handlerType))
    run :: RunCallback (APIGatewayHandlerType) context 
    run opts  = do
