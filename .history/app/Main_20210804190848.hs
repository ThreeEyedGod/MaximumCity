...
main =  do
      ...
      res <- getTelegramSettings --| res comes back as 
      case res of 
                  ...
                  | not (Prelude.null (unpack s)) = do -- no telegram token. Route it to bad handler
                        case (eitherDecode (eventObject opts)) of  
                              ...
                              Right inComingEvent -> do
                                    result <- badhandler (unpack s) inComingEvent (contextObject opts)
                                    return . pure $ result
                  | otherwise = do -- OK case regular handler will deal with it
                        case (eitherDecode (eventObject opts)) of  
                              Left _  -> error "Fail "
                              Right inComingEvent -> do
                                    result <- handler tc inComingEvent (contextObject opts)
                                    return . pure $ result
