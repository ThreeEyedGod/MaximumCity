...
main =  do
      ...
      where
            ...
            run s tc opts
                  | not (Prelude.null (pack s) = do -- no telegram token. Route it to bad handler
                        result <- badhandler (pack s (eventObject opts) (contextObject opts)
                        pure . APIGatewayResult $ mkApiGatewayResponse result
                  | otherwise = do -- OK case regular handler will deal with it
                        result <- handler tc (eventObject opts) (contextObject opts)
                        pure . APIGatewayResult $ mkApiGatewayResponse result
{-- 
